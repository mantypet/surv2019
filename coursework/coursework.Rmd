...
$$
f(t)_{exp}= \lambda\exp(-\lambda t)
$$

And with a *standard* parametrization $f(t;\lambda,k)$ for the Weibull distribution is

$$
\frac{k}{\lambda}\Bigg(\frac{t}{\lambda}\Bigg)^{k-1}\exp(-(t/\lambda)^k)
$$

There are alternative parametrizations such as $f(t;\beta,k)$ with $\beta = \frac{1}{\lambda}$ which yields

$$
k\beta t^{k-1} \exp(-\beta t^k)
$$

The one we will be using is $f(t;\alpha,p)$ (which I hopelessly tried to derive) $p=(\frac{1}{\lambda})^k$ and $p\alpha=k(\frac{1}{\lambda})^k$ perhaps?

$$
p \alpha^p t^{p-1} \exp(-(\alpha t)^p)
$$

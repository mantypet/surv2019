...
The one in the course material, which we will be using is $f(t;\alpha,p)$ (which I failed to derive I'm afraid) is
...
Now just for practice, let's show that $S(t)=\int_t^\infty \lambda\exp(-\lambda u)du$

$$
\begin{aligned}
S(t)=\int_t^\infty \lambda\exp(-\lambda u)du &= 1 - \int_0^t \lambda\exp(-\lambda u)du \\
&=1-\mid_0^t
\end{aligned}
$$

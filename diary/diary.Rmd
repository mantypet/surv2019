...
Say we are interested in making inferences about an unknown parameter $\theta$ from information provided by sampling a set of random variables $X_1,\dots,X_n$ or $\mathbf{X}$. A usual prospect is to try to summarize the information in the sample $x_1,\dots,x_n$ or $\mathbf{x}$ (the realization of $\mathbf{X}$) regarding the parameter of interest $\theta$ by some function of the sample $T(\mathbf{x})$, probably the most common ones being sample means, sample variances, sample totals.
...
That is, if we have two samples $\mathbf{x}$ and $\mathbf{y}$ and $T(\mathbf{x})=T(\mathbf{y})$ any conclusions about $\theta$ should not differ whether we observe $\mathbf{X}=\mathbf{x}$ or $\mathbf{Y}=\mathbf{y}$.

In other words (for example in a discrete case) show that $P_\theta(\mathbf{X}=\mathbf{x})=P_\theta(\mathbf{Y}=\mathbf{y}),\quad \forall x,\theta$ 
